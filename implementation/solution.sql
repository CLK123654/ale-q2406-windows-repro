...
BEGIN
  IF EXISTS(
    SELECT 1 FROM release_ops.release_window left_window
    JOIN release_ops.release_window right_window
...
BEGIN
  PERFORM 1 FROM release_ops.release_window WHERE window_id IN(p_left_window,p_right_window) AND status='ACTIVE' ORDER BY window_id FOR UPDATE;
  SELECT slot,lane_id INTO STRICT left_slot,left_lane FROM release_ops.release_window WHERE window_id=p_left_window AND status='ACTIVE';
  SELECT slot,lane_id INTO STRICT right_slot,right_lane FROM release_ops.release_window WHERE window_id=p_right_window AND status='ACTIVE';
  IF left_lane<>right_lane THEN RAISE EXCEPTION 'release windows must belong to one lane'; END IF;
  UPDATE release_ops.release_window SET slot=right_slot WHERE window_id=p_left_window;
  UPDATE release_ops.release_window SET slot=left_slot WHERE window_id=p_right_window;
END;
$$;
...
RETURNS void LANGUAGE plpgsql AS $$
BEGIN
  UPDATE release_ops.release_window SET slot=p_slot WHERE window_id=p_window_id AND status='ACTIVE';
...
RETURNS void LANGUAGE plpgsql AS $$
BEGIN
  UPDATE release_ops.release_window SET status='CANCELLED' WHERE window_id=p_window_id AND status='ACTIVE';
  IF NOT FOUND THEN RAISE EXCEPTION 'active release window not found: %',p_window_id; END IF;
END;
$$;
...
BEGIN
  PERFORM 1 FROM release_ops.release_lane WHERE lane_id=p_lane_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'release lane not found: %',p_lane_id; END IF;
  IF EXISTS(SELECT 1 FROM release_ops.release_window WHERE lane_id=p_lane_id AND status='ACTIVE' AND slot && p_slot) THEN
    UPDATE release_ops.pending_window SET decision='BLOCKED_OVERLAP',decision_tx=p_tx_id
    WHERE lane_id=p_lane_id AND requested_slot=p_slot AND decision='PENDING';
    RETURN 'BLOCKED_OVERLAP';
  END IF;
  SELECT pending_id INTO winner FROM release_ops.pending_window
  WHERE lane_id=p_lane_id AND requested_slot=p_slot AND decision='PENDING'
  ...
  INSERT INTO release_ops.release_window(window_id,lane_id,release_ref,slot,status,source_pending_id)
  SELECT promoted_id,lane_id,release_ref,requested_slot,'ACTIVE',pending_id FROM release_ops.pending_window WHERE pending_id=winner;
  UPDATE release_ops.pending_window SET decision=CASE WHEN pending_id=winner THEN 'PROMOTED' ELSE 'LOST_PRIORITY' END,
